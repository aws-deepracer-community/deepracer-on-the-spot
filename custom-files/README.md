...
* `DR_IMPORT_MODEL_REGULAR_UPLOAD`: DeepRacer on the Spot specific var.  Boolean defining whether to upload the model to the DeepRacer Console at the interval defined by  `DR_REGULAR_UPLOAD`.  this is useful if you want to regularly test a checkpoint of your model in the console.  Default is to not regularly import models, set the variable to True to disable this.  So for example if you want to upload a model to the console once per hour set `DR_REGULAR_UPLOAD` to 60 and `DR_IMPORT_MODEL_REGULAR_UPLOAD` to True.
